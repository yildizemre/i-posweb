...
import { Car, Tag, ArrowRight, Percent, TrendingUp, MapPin } from 'lucide-react'
...
import {
  PLATES, getTripsByPlate, formatMoney,
        ...
        <div className={s.chartCard} style={{ marginBottom: 20 }}>
            ...
            return (
              ...
              >
                <div>
                  ...
                  <span>{p.city} · Sahip {p.owner} · Sürücü {p.driver} · {trips} yolculuk · {p.customers} müşteri</span>
                </div>
          ...
          {plate && plate.platform === selectedPlatform && (
            <div className={s.plateDetail}>
              <h4>{plate.plate} — Sahip {plate.owner} · Sürücü {plate.driver}</h4>
...
export default function PanelHome() {
  const { isAdmin, isUser, isOwner } = useAuth()
  if (isAdmin) return <AdminPanelHome />
  if (isUser || isOwner) return <OwnerHomePage />
