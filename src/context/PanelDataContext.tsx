...
import { OWNER_WALLET_GROUPS, OwnerWalletGroup, SubWalletDriverAssign } from '../utils/ownerWallets'
...
export const ADMIN_PAYOUT_WALLETS = [
  { walletId: 'CZD-ADMIN-01', label: 'ABC Admin Cüzdanı' },
...
const INITIAL_WALLETS: DriverWallet[] = OWNER_WALLET_GROUPS.flatMap((g) =>
  g.subs
    .filter((s) => s.driver && s.driverWalletId)
    .map((s) => ({
      id: s.id,
      walletId: s.driverWalletId!,
      driver: s.driver!,
      tc: s.driverTc ?? '',
      phone: s.driverPhone ?? '',
      plate: s.plate,
      platform: PLATFORMS.find((p) => p.id === g.platform)?.name ?? '',
      balance: s.balance,
      status: s.status,
...
const INITIAL_CAMPAIGNS: Campaign[] = [
  {
    ...
    description: '4 adet kış lastiği — %20 indirimli fiyat üzerinden ABC Anonim ile özel satış.',
  ...
  },
  {
    ...
    payoutWalletId: 'CZD-A-01', startDate: '01.06.2025', endDate: '30.09.2025', status: 'active',
  },
  {
...
const PanelDataContext = createContext<{
  ...
  ownerWalletGroups: OwnerWalletGroup[]
  getOwnerWallets: (ownerId: string) => OwnerWalletGroup | undefined
  assignSubWalletDriver: (
    subWalletId: string,
    data: SubWalletDriverAssign,
...
export function PanelDataProvider({ children }: { children: ReactNode }) {
  ...
  const [ownerWalletGroups, setOwnerWalletGroups] = useState(OWNER_WALLET_GROUPS)
  ...
  const getOwnerWallets = (ownerId: string) => ownerWalletGroups.find((g) => g.ownerId === ownerId)

  const assignSubWalletDriver = (subWalletId: string, data: SubWalletDriverAssign) => {
    const name = data.fullName.trim()
    const tc = data.tc.replace(/\D/g, '')
    const phone = data.phone.trim()
    const walletId = data.walletNo.trim().toUpperCase()

    if (!name) return { ok: false, error: 'Ad soyad giriniz.' }
    if (!tc) return { ok: false, error: 'TC kimlik no giriniz.' }
    if (!/^\d{11}$/.test(tc)) return { ok: false, error: 'TC kimlik no 11 haneli olmalıdır.' }
    if (!phone) return { ok: false, error: 'Telefon giriniz.' }
    if (!walletId) return { ok: false, error: 'Cüzdan no giriniz.' }
    if (!/^CZD-[A-Z0-9-]+$/.test(walletId)) {
      return { ok: false, error: 'Cüzdan no CZD- ile başlamalıdır. Örn: CZD-100042' }
    }

    const group = ownerWalletGroups.find((g) => g.subs.some((s) => s.id === subWalletId))
    const sub = group?.subs.find((s) => s.id === subWalletId)
    if (!sub || !group) return { ok: false, error: 'Alt cüzdan bulunamadı.' }
    if (sub.driver) return { ok: false, error: 'Bu cüzdana zaten şoför atanmış.' }
    if (group.subs.some((s) => s.driver?.toLowerCase() === name.toLowerCase())) {
      return { ok: false, error: 'Bu isimde şoför zaten atanmış.' }
    }

    const allSubs = ownerWalletGroups.flatMap((g) => g.subs)
    if (allSubs.some((s) => s.driverTc === tc)) {
      return { ok: false, error: 'Bu TC ile kayıtlı şoför zaten var.' }
    }
    const allDriverWalletIds = allSubs.map((s) => s.driverWalletId).filter(Boolean)
    if (wallets.some((w) => w.walletId === walletId) || allDriverWalletIds.includes(walletId)) {
      return { ok: false, error: 'Bu cüzdan no zaten kullanılıyor.' }
    }

    setOwnerWalletGroups((prev) => prev.map((g) => (
    ...
    const platformName = PLATFORMS.find((p) => p.id === group.platform)?.name ?? ''
    setWallets((prev) => {
      const existing = prev.find((w) => w.walletId === walletId)
      if (existing) {
        return prev.map((w) => (
          w.walletId === walletId ? { ...w, driver: name, tc, phone } : w
        ))
      }
      return [...prev, {
        id: newId(),
  ...
  return (
    <PanelDataContext.Provider value={{
      ...
      wallets, ownerWalletGroups, getOwnerWallets, assignSubWalletDriver,
      addWallet, deleteWallet, updateWallet, driversWithoutWallet,
