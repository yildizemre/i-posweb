...
export default function DashboardLayout() {
  ...
  const { isAdmin, isUser, isOwner, ownerId } = useAuth()
  ...
  const userFleetNav = (
    <nav className={styles.nav}>
      ...
      {link('/panel/cuzdanim', 'Cüzdanım', Wallet)}
      {link('/panel/firsatlar', 'Fırsatlar & Kampanyalar', Tag)}
  ...
  const ownerNav = (
    <nav className={styles.nav}>
      {link('/panel', 'Panel', LayoutDashboard)}
      <p className={styles.navSection}>FİLOM</p>
      {link('/panel/cuzdanim', 'Cüzdanım', Wallet)}
      {link('/panel/firsatlar', 'Fırsatlar & Kampanyalar', Tag)}
      <p className={styles.navSection}>Daha Fazla</p>
      {link('/panel/destek', 'Yardım ve Destek', Headphones)}
  ...
  const roleLabel = isAdmin ? 'Admin' : isUser ? 'Filo Sahibi' : isOwner ? `Taksi Sahibi ${ownerId}` : 'Kullanıcı'
  const headerName = isUser || isOwner ? profile.name : COMPANY_NAME
  ...
  return (
    <div className={styles.layout}>
      ...
      <aside className={`${styles.sidebar} ${menuOpen ? styles.sidebarOpen : ''}`}>
        ...
        {isAdmin ? adminNav : isUser ? userFleetNav : isOwner ? ownerNav : userFleetNav}
      </aside>
